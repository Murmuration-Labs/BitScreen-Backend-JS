...
describe("Get address hash", () => {
    ...
    it("Should return the hash of the address", async () => {
        ...
        expect(result).toBe("d49b844ffe4d4f7947fb493c9f60d0d15bac9b8d5c4eae4c830f6754a8b2d7bc")
    })
})
...
describe("Generate random hex", () => {
    ...
    it("Should throw error on negative length", async () => {
        try {
            ...
            expect(error.message).toBe("The value of \"size\" is out of range. It must be >= 0 && <= 2147483647. Received -5")
            return;
        }
...
describe("Generate random token", () => {
    ...
    it("Should return a random token with 4 bits (default)", async () => {
        jest.spyOn(crypto, 'generateRandomHex').mockResolvedValueOnce('1234').mockResolvedValueOnce('abcd').mockResolvedValueOnce('keyk').mockResolvedValueOnce('btsc')
    ...
    it("Should return a random token with 3 bits (custom)", async () => {
        jest.spyOn(crypto, 'generateRandomHex').mockResolvedValueOnce('1234').mockResolvedValueOnce('keyk').mockResolvedValueOnce('btsc')
