...
import {create_deal, get_deal_stats} from "../../src/controllers/deal.controller";
import {Deal, DealStatus, DealType} from "../../src/entity/Deal";
import {
    addEndInterval,
    addStartInterval,
    BucketSize, fillDates,
    getCidForProvider,
    getStatsBaseQuery, setBucketSize
...
describe("Deal Controller: POST /deal", () => {
    ...
    it("Should throw error on provider not found", async () => {
        ...
        expect(providerRepo.findOne).toHaveBeenCalledWith({walletAddressHashed: '4c8f18581c0167eb90a761b4a304e009b924f03b619a0c0e8ea3adfce20aee64'})
    ...
    it("Should throw error on cid not found", async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(400)
        expect(res.send).toHaveBeenCalledTimes(1)
        expect(res.send).toHaveBeenCalledWith({message: "CID is not present in any of this provider's filters."})
    })
...
describe("Deal Controller: GET /deal/stats/:bucketSize", () => {
    ...
    it("Should get stats without interval", async () => {
        const req = getMockReq({
            ...
            query: {},
            body: {
                walletAddressHashed: '4c8f18581c0167eb90a761b4a304e009b924f03b619a0c0e8ea3adfce20aee64'
            }
        })
        ...
        const statsQuery = {
            getRawMany: jest.fn().mockResolvedValueOnce([{value: 1, key: 'test1'}, {value: 2, key: 'test2'}])
        }
