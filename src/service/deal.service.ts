...
export const getStatsBaseQuery = (providerId: number): SelectQueryBuilder<Deal> => {
...
export const addStartInterval = (query: SelectQueryBuilder<Deal>, start): SelectQueryBuilder<Deal> => {
    query.andWhere("to_char(d.created, 'YYYY-MM-DD') >= :startDate", {startDate: start});

    return query
}

export const addEndInterval = (query: SelectQueryBuilder<Deal>, end): SelectQueryBuilder<Deal> => {
    query.andWhere("to_char(d.created, 'YYYY-MM-DD') <= :endDate", {endDate: end});

    return query
}

export const setBucketSize = (query: SelectQueryBuilder<Deal>, bucketSize): SelectQueryBuilder<Deal> => {
    switch (bucketSize) {
